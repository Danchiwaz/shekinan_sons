...
const Navbar: React.FC = () => {
    ...
    return (
        <nav className="fixed top-0 left-0 right-0 z-50">
            <motion.div
                className={`${isScrolled ? 'bg-white dark:bg-gray-900/85 backdrop-blur-xl nav-glow' : 'bg-transparent'} transition-colors duration-300`}
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
            ...
            >
                <div className="container mx-auto px-4 py-3 flex justify-between items-center">
                    ...
                    <div className="hidden md:flex items-center space-x-6">
                        <a href="#home" className={`nav-link ${isScrolled ? 'text-gray-900 dark:text-white hover:text-amber-600' : 'text-white/90 hover:text-white'}`}>Home</a>
                        <a href="#about" className={`nav-link ${isScrolled ? 'text-gray-900 dark:text-white hover:text-amber-600' : 'text-white/90 hover:text-white'}`}>About</a>
                        <a href="#events" className={`nav-link ${isScrolled ? 'text-gray-900 dark:text-white hover:text-amber-600' : 'text-white/90 hover:text-white'}`}>Events</a>
                        <a href="#sermons" className={`nav-link ${isScrolled ? 'text-gray-900 dark:text-white hover:text-amber-600' : 'text-white/90 hover:text-white'}`}>Sermons</a>
                        <a href="#gallery" className={`nav-link ${isScrolled ? 'text-gray-900 dark:text-white hover:text-amber-600' : 'text-white/90 hover:text-white'}`}>Gallery</a>
                        <a href="#devotion" className={`nav-link ${isScrolled ? 'text-gray-900 dark:text-white hover:text-amber-600' : 'text-white/90 hover:text-white'}`}>Daily Devotion</a>
                        <a href="#contact" className={`nav-link ${isScrolled ? 'text-gray-900 dark:text-white hover:text-amber-600' : 'text-white/90 hover:text-white'}`}>Contact</a>
                        <button aria-label="Toggle theme" onClick={toggleTheme} className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-muted transition">
                            {isDark ? <Sun size={18} className={`${isScrolled ? 'text-gray-900 dark:text-white' : 'text-white'}`}/> : <Moon size={18} className={`${isScrolled ? 'text-gray-900' : 'text-white'}`}/>}
                        </button>
                    ...
                    <button
                        ...
                        className={`md:hidden focus:outline-none ${isScrolled ? 'text-gray-900 dark:text-white' : 'text-white'}`}
